---
title: Introduction
---

<div className="mx-auto mt-5 text-black dark:text-white text-center">
  {/* Light theme: dark-colored logo. Toggled purely via Tailwind's `dark:`
      variant (wired to the `.dark` class in globals.css), so it's SSR-safe
      with no theme-switch flash. */}
  <img
    className="block dark:hidden"
    style={{ width: '75%', margin: '0 auto 2em auto' }}
    src="https://github.com/reaviz/reagraph/raw/master/stories/assets/logo-light.png"
    alt="Reagraph"
  />
  {/* Dark theme: white logo */}
  <img
    className="hidden dark:block"
    style={{ width: '75%', margin: '0 auto 2em auto' }}
    src="https://github.com/reaviz/reagraph/raw/master/stories/assets/logo.png"
    alt="Reagraph"
  />
  <br />
  <br />
  <div className="flex flex-row gap-2 justify-center">
  <a href="https://github.com/reaviz/reagraph/workflows/build/" target="_blank">
    <img src="https://github.com/reaviz/reagraph/workflows/build/badge.svg?branch=master" />
  </a>
  <a href="https://npm.im/reagraph" target="_blank">
    <img src="https://img.shields.io/npm/v/reagraph.svg" />
  </a>
  <a href="https://npm.im/reagraph" target="_blank">
    <img src="https://badgen.net/npm/dw/reagraph" />
  </a>
  <a href="https://github.com/reaviz/reagraph/blob/master/LICENSE" target="_blank">
    <img src="https://badgen.now.sh/badge/license/apache2" />
  </a>
  <a href="https://github.com/reaviz/reagraph" target="_blank">
    <img alt="GitHub stars" src="https://img.shields.io/github/stars/reaviz/reagraph?style=social" />
  </a>
  <a href="https://discord.gg/tt8wGExq35" target="_blank">
    <img src="https://img.shields.io/discord/773948315037073409?label=discord" />
  </a>
  </div>

  Reagraph is a high-performance network graph visualization built in WebGL for React.

</div>

## ✨ Features

- **WebGL based** for high performance
- **Node Sizing** based on attribute, page rank, centrality, custom
- **Light and Dark Mode** with custom theme ability
- **Path finding** between nodes
- **Radial Context Menu**
- **Highlight and Selection Hook**
- **Dragging Nodes**
- **Lasso Selection**
- **Expand/Collapse Nodes**
- **Customizable Nodes**
- **Advanced Label Placement**
- **Edge Interpolation**
- **Clustering**
